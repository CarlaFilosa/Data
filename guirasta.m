...
function [psth trialspx triallx o] = guirasta(d, unit,trigtimes,varargin)
%%
...
% if length(trigtimes)>25
%   trigtimes=trigtimes(end-24:end);
% end
...
elseif chart==2
%   
% rectpos=[pre 0 odor_dur max(ylim)];
% rc=rectangle('pos',rectpos,'facecolor',[.8 .8 .8],'edgecolor','none');

% set(gca,'children',flipud(get(gca,'children'))) %reverse order of objects
...
yyaxis left
%    axes('DrawMode', 'fast');
ptch=patch([pre pre pre+odor_dur pre+odor_dur],[0 length(trigtimes)   length(trigtimes) 0], 'k');
  set(ptch,'FaceAlpha',0.2);
...
% yyaxis left
    ...
    rasterplot_pj(find(rastmat'), find(lastmat'), find(kastmat'),rrastmat,crastmat)
...
%     axis([min(psth(:,1))-10 max(psth(:,1))+10 0 max(psth(:,2))+1])
    ...
    %axis([min(psth(:,1))-10 max(psth(:,1))+10 0 25])
         ...
         plot(psth(:,1)+pre,psth(:,2), 'Color', [1 .5 0], 'LineWidth', 3)
...
set(gca, 'YColor', 'k');
line([lick_delay+pre lick_delay+pre], [0 (max(ylim))], 'color', 'r', 'linewidth', 1)
...
function rasterplot_pj(times,times2, times3, numtrials,triallen, varargin)
...
%%%%%%%%% Code Begins %%%%%%%%%%%%
switch nin
    case 5 %no handle so plot in a separate figure
        
        hresp=gca;
        fs=defaultfs;
    case 6 %handle supplied
        hresp=varargin{1};
        if (~ishandle(hresp))
...
hold on
plot(xx2,yy2,plotcolor_licks, 'linewidth', plotwidth);
plot(xx3,yy3,plotcolor_licks, 'linewidth', plotwidth+1, 'color', 'b', 'LineStyle', '-');
hold on
